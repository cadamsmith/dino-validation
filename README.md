...
- [Localization](#localization)
  - [Changing Default Messages](#changing-default-messages)
  - [Using Localization Builds](#using-localization-builds)
  - [Creating Custom Language Files](#creating-custom-language-files)
...
| **UMD** | Browsers, legacy builds, CDN             | `<script src="dist/dv.umd.js"></script>` |
...
```html
<script src="node_modules/dino-validation/dist/dv.umd.js"></script>
<script>
  dv.validate('#myForm', {
    rules: {
      email: { required: true, email: true },
...
<script src="dist/localization/messages_fr.umd.js"></script>
...
A complete user registration form with validation, custom styling, and error handling.
    ...
    <script>
      ...
      document
        ...
        .addEventListener('submit', (e) => {
          if (!dv.valid('#registrationForm')) {
            e.preventDefault();
          } else {
            // Form is valid, would submit here
            console.log('Form is valid, ready to submit!');
            e.preventDefault(); // Remove this to actually submit
          }
        });
    </script>
  </body>
...
A product review form with async validation and loading states.
...
<html lang="en">
  <head>
    ...
    <style>
      /* Similar styles as registration form */
      .form-group {
        margin-bottom: 1rem;
      }
      .error {
    ...
    <form id="reviewForm">
      <div class="form-group">
        ...
        <select id="productId" name="productId" required>
          ...
          <option value="3">Widget C</option>
        </select>
      </div>
    ...
    <script>
      ...
      dv.addMethod(
        ...
        async function ({ blank, value }) {
          ...
          // For demo: reject if title contains "duplicate"
          return !value.toLowerCase().includes('duplicate');
        },
        'You have already submitted a review with this title',
      );

      dv.validate('#reviewForm', {
        rules: {
        ...
        messages: {
          ...
          comment: {
            ...
            minlength: 'Review must be at least 20 characters',
            maxlength: 'Review cannot exceed 5000 characters',
          },
        },
        invalidHandler: (event) => {
          const status = document.getElementById('submitStatus');
          status.className = 'error-message';
          status.textContent = `Please fix ${event.detail.length} error(s) before submitting`;
        },
      });
