...
test("elements() order", async ({ page }) => {
  ...
  expect(result[0]).toEqual(["order1", "order2", "order3", "order4", "order5", "order6"]);
  expect(result[1]).toEqual(["order1-error", "order2-error", "order3-error", "order4-error", "order5-error", "order6-error"]);
});
